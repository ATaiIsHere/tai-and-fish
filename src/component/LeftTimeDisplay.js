...
const remainTime = (target) => {
  let now = new Date();
  let remainSeconds = Math.abs(target.getTime() - now.getTime()) / 1000;
...
const LeftTimeDisplay = (props) => {
  ...
  useEffect(() => {
    let repeat = setInterval(() => {
      let [remainDays, remainHour, remainMins, remainSecs] = remainTime(
        props.target
      );
  ...
  return (
    <div className="lefttime-display">
      ...
      <DigitalBall digital={secs} unit="SECONDS" keep={true} />
    </div>
  );
};
